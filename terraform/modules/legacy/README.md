...
Deleting this directory was proposed in [hackforla/incubator#164](https://github.com/hackforla/incubator/issues/164) and deliberately declined — it is the best surviving record of how the earlier platform was assembled. The abandoned Terragrunt state files it paired with, which lived under `s3://hlfa-incubator-terragrunt/terragrunt-states/incubator/projects-*/`, were stale enough that this code is often the clearer account of intent. That bucket was deleted in [hackforla/incubator#170](https://github.com/hackforla/incubator/issues/170) and its state files archived to the DevOps CoP Google Drive first, so they are no longer retrievable from AWS — which makes this directory the only copy of the legacy platform's shape that lives in version control.
